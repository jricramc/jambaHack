...
const initialPositions = [
  {
    ...
    name: 'Technology',
    items: [
      { id: 'AAPL1', name: 'AAPL', type: 'stock', quantity: 100, delta: 100, gamma: 0, theta: 0, vega: 0, fv: 15025, tradePrice: 150.25 },
      { id: 'GOOGL1', name: 'GOOGL', type: 'option', optionType: 'Call', strike: 2800, expiry: '2023-08-18', quantity: 1, delta: 0.6, gamma: 0.001, theta: -1.5, vega: 0.3, fv: 150, tradePrice: 150 },
      { id: 'MSFT1', name: 'MSFT', type: 'stock', quantity: 50, delta: 50, gamma: 0, theta: 0, vega: 0, fv: 15257.5, tradePrice: 305.15 },
    ],
  },
  {
    id: 'finance',
    name: 'Finance',
    items: [
      { id: 'JPM1', name: 'JPM', type: 'stock', quantity: 75, delta: 75, gamma: 0, theta: 0, vega: 0, fv: 11647.5, tradePrice: 155.30 },
      { id: 'GS1', name: 'GS', type: 'option', optionType: 'Call', strike: 350, expiry: '2023-10-20', quantity: 1, delta: 0.5, gamma: 0.001, theta: -1.2, vega: 0.25, fv: 180, tradePrice: 180 },
      { id: 'BAC1', name: 'BAC', type: 'stock', quantity: 100, delta: 100, gamma: 0, theta: 0, vega: 0, fv: 3000, tradePrice: 30.00 },
    ],
  },
]
...
const GreekDisplay = ({ greeks }) => (
  <div className="grid grid-cols-4 gap-1 text-xs text-right">
    <span>{greeks.delta.toFixed(2)}</span>
    <span>{greeks.gamma.toFixed(3)}</span>
    <span>{greeks.theta.toFixed(2)}</span>
    <span>{greeks.vega.toFixed(2)}</span>
  </div>
)
...
const PnLDisplay = ({ item }) => (
  <div className="grid grid-cols-4 gap-1 text-xs text-right">
    ...
    <span>{((item.fv / item.tradePrice - 1) * 100).toFixed(2)}%</span>
...
const PositionItem = ({ item, index, onSelectInstrument, showGreeks }) => {
  ...
  const getDisplayName = () => {
    if (item.type === 'stock') {
      ...
      return `${item.name} (${formattedDate} ${item.strike}${optionType})`;
    }
  };
...
const FolderItem = ({ folder, index, onSelectInstrument, showGreeks }) => {
  ...
  const totalGreeks = folder.items.reduce((acc, item) => ({
    delta: acc.delta + item.delta * item.quantity,
    gamma: acc.gamma + item.gamma * item.quantity,
    theta: acc.theta + item.theta * item.quantity,
    vega: acc.vega + item.vega * item.quantity,
    fv: acc.fv + item.fv * item.quantity,
  }), { delta: 0, gamma: 0, theta: 0, vega: 0, fv: 0 })

  const totalPnL = folder.items.reduce((acc, item) => ({
    tradePrice: acc.tradePrice + item.tradePrice * item.quantity,
...
const PositionsPanel = ({ positions, onDragEnd, onSelectInstrument, isCollapsed, onToggleCollapse }) => {
  const [showGreeks, setShowGreeks] = useState(false)

  const calculatePnL = () => {
    return positions.reduce((total, folder) => {
      return total + folder.items.reduce((folderTotal, item) => {
        const currentPrice = stocksData.find(stock => stock.symbol === item.name)?.price || item.tradePrice
        const pnl = item.quantity * (currentPrice - item.tradePrice)
        return folderTotal + pnl
      }, 0)
    }, 0)
  }
  ...
  const calculateAccountValue = () => {
    return positions.reduce((total, folder) => {
      return total + folder.items.reduce((folderTotal, item) => {
        return folderTotal + item.quantity * item.fv
      }, 0)
    }, 0)
  }
  ...
  return (
    <div className={`transition-all duration-300 ${isCollapsed ? 'w-12' : 'w-72'}`}>
        ...
        <>
          ...
          <ScrollArea className="h-[calc(100vh-16rem)]">
            <div className="mb-2 flex items-center text-xs font-medium text-muted-foreground">
              ...
              <div className="flex-shrink-0 w-40 grid grid-cols-4 gap-1 text-right">
                {showGreeks ? (
                  <>
                    <span>Δ</span>
                    <span>Γ</span>
...
const SpreadBuilder = ({ onCancel, onViewSpread, onExecuteOrder, spread, addToSpread }) => {
  ...
  const calculateNetTheoreticalValue = () => {
    return spread.reduce((acc, item) => acc + item.fv * item.quantity, 0)
  }
            ...
            <TableBody>
              {spread.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{item.delta.toFixed(2)}</TableCell>
                  <TableCell>{item.gamma.toFixed(3)}</TableCell>
                  <TableCell>{item.theta.toFixed(2)}</TableCell>
                  <TableCell>{item.vega.toFixed(2)}</TableCell>
...
const DeskChat = ({ messages, onSendMessage, onMessageClick, isCollapsed, onToggleCollapse, unreadCount }) => {
  ...
  return (
    <div className={`flex flex-col transition-all duration-300 ${isCollapsed ? 'h-12' : 'h-64'}`}>
...
const RoleLegend = ({ roles, onSelectAgent }) => {
  return (
  ...
  const handleExecuteOrder = (spread, orderType, limitPrice) => {
    ...
    spread.forEach(item => {
      const folderIndex = newPositions.findIndex(folder => 
        folder.items.some(position => position.name === item.symbol)
      )
      if (folderIndex !== -1) {
        const itemIndex = newPositions[folderIndex].items.findIndex(position => 
          position.name === item.symbol &&
        ...
        if (itemIndex !== -1) {
          newPositions[folderIndex].items[itemIndex].quantity += item.quantity
        } else {
          newPositions[folderIndex].items.push({
  ...
  const renderMainContent = () => {
    if (viewingSpread) {
      ...
      const pnl = positions.reduce((total, folder) => {
        return total + folder.items.reduce((folderTotal, item) => {
          const currentPrice = stocksData.find(stock => stock.symbol === item.name)?.price || item.tradePrice
          return folderTotal + item.quantity * (currentPrice - item.tradePrice)
        }, 0)
      }, 0)
      const pnlPercentage = (pnl / accountValue) * 100
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <Tabs value={selectedRole} onValueChange={setSelectedRole} className="w-full">
            <TabsList className="grid w-full grid-cols-4">
              {roles.map((role) => (
                <TabsTrigger key={role.name} value={role.name} className="relative">
                  <div className={`w-3 h-3 rounded-full absolute left-2`} style={{ backgroundColor: role.color }}></div>
                  {role.name}
                </TabsTrigger>
              ))}
            </TabsList>
        ...
        <aside className={`border-l transition-all duration-300 ${isToolsPanelCollapsed ? 'w-12' : 'w-64'}`}>
          <div className="p-4 flex flex-col h-full">
            ...
            <RoleLegend roles={roles} onSelectAgent={setSelectedAgent} />
          </div>
        </aside>
